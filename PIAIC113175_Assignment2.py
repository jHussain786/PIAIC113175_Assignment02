...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = np.hstack((np.vstack((arr,arr,arr)).reshape(9, order='F'),arr,arr,arr))#write your code here 
...
def function13():
    ...
    ans = np.array(arr[np.logical_and(arr>5, arr<10)])#write your code here 
...
def function18():
    ...
    x = np.count_nonzero(np.searchsorted(piaic,students)<np.size(piaic,0)) # Write you code Here
    return x
...
def function19():
    ...
    output = np.add(np.multiply(X,W),b)   # Write your code here
